...
def get_agent_stats(hc_home: Path, agent: str) -> dict:
    ...
    all_tasks = list_tasks(hc_home, assignee=agent)
...
def main():
    ...
    if args.command == "log":
        msg_id = log_message(args.home, args.sender, args.recipient, args.msg)
        print(f"Logged message #{msg_id}")
    elif args.command == "event":
        msg_id = log_event(args.home, args.msg)
        ...
        messages = get_messages(args.home, since=args.since, between=between, msg_type=args.msg_type, limit=args.limit)
        for m in messages:
