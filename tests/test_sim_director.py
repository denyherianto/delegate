"""Tests for scripts/sim_director.py — simulated director for eval runs."""
...
from scripts.mailbox import send as mailbox_send, read_inbox, deliver, Message
from scripts.sim_director import (
    sim_director_respond,
    run_sim_director,
    start_sim_director_thread,
...
def team_root(tmp_path):
    """Create a bootstrapped team directory for sim-director tests."""
    root = tmp_path / "team"
    bootstrap(root, manager="manager", director="director", agents=["alice"])
    return root
...
async def mock_llm_echo(prompt: str) -> str:
    ...
    return f"Sim-director response to: {prompt[:60]}..."
...
class TestBuildPrompt:
    ...
    def test_uses_template_format(self):
        ...
        assert "You are a director" in prompt
...
# ---------------------------------------------------------------------------
# Unit tests: sim_director_respond
# ---------------------------------------------------------------------------


class TestSimDirectorRespond:
    """Tests for sim_director_respond() with mocked LLM."""

    @pytest.mark.asyncio
    async def test_returns_string(self, team_root):
        """Returns a string response."""
        result = await sim_director_respond(
            team_root,
...
class TestProcessInbox:
    ...
    async def test_processes_unread_messages(self, team_root):
        """Processes messages in the director's inbox and sends responses."""
        # Send a message from manager to director
        _deliver_to_inbox(team_root, "manager", "director", "What should I work on?")
        ...
        # Director's inbox should now be empty (messages marked read)
        remaining = read_inbox(team_root, "director", unread_only=True)
        assert len(remaining) == 0

    @pytest.mark.asyncio
    async def test_sends_response_to_sender(self, team_root):
        """The response is routed back to the message sender."""
        _deliver_to_inbox(team_root, "manager", "director", "How do I start?")
        ...
        # The response should be in director's outbox (pending routing)
        from scripts.mailbox import read_outbox
        outbox = read_outbox(team_root, "director", pending_only=True)
        assert len(outbox) == 1
        assert outbox[0].recipient == "manager"
        assert outbox[0].sender == "director"
    ...
    async def test_processes_multiple_messages(self, team_root):
        ...
        _deliver_to_inbox(team_root, "manager", "director", "Question 1")
        _deliver_to_inbox(team_root, "alice", "director", "Question 2")
    ...
    async def test_no_specs_returns_fallback(self, team_root):
        ...
        _deliver_to_inbox(team_root, "manager", "director", "What now?")
...
# ---------------------------------------------------------------------------
# Integration tests: run_sim_director polling loop
# ---------------------------------------------------------------------------


class TestRunSimDirector:
    """Tests for run_sim_director() — the full polling loop."""
    ...
    async def test_processes_messages_before_stopping(self, team_root):
        ...
        _deliver_to_inbox(team_root, "manager", "director", "Help me with task")
        ...
        # Director inbox should be cleared
        remaining = read_inbox(team_root, "director", unread_only=True)
        assert len(remaining) == 0
...
class TestStartSimDirectorThread:
    """Tests for start_sim_director_thread()."""

    def test_starts_and_stops(self, team_root):
        """Thread starts and can be stopped cleanly."""
        thread, stop_event = start_sim_director_thread(
            team_root,
            task_specs={"Task": "Desc"},
            poll_interval=0.1,
            llm_query=mock_llm_echo,
    ...
    def test_processes_messages_in_thread(self, team_root):
        ...
        _deliver_to_inbox(team_root, "manager", "director", "Thread test question")
