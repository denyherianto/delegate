...
When a task is rejected by the human director or hits a merge conflict,
...
def notify_rejection(
    ...
    try:
        ...
        logger.info(
            "Rejection notification sent for T%04d to %s", task_id, manager
        )
        return filename
    except (ValueError, FileNotFoundError) as e:
        logger.warning(
            "Failed to send rejection notification for T%04d: %s", task_id, e
        )
        return None
...
def notify_conflict(
    ...
    body = (
        f"MERGE_CONFLICT: T{task_id:04d}\n"
        f"\n"
        f"Task: T{task_id:04d} — {title}\n"
        f"Branch: {branch}\n"
        f"Assignee: {assignee}\n"
    ...
    try:
        filename = deliver(hc_home, team, msg)
        logger.info(
            "Conflict notification sent for T%04d to %s", task_id, manager
        )
        return filename
    except (ValueError, FileNotFoundError) as e:
        logger.warning(
            "Failed to send conflict notification for T%04d: %s", task_id, e
        )
        return None
